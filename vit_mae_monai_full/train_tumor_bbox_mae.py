...
"""
Self‑supervised tumor-bbox MAE pre‑training
--------------------------------------------------------------------------------
Usage:
    python train_tumor_mae.py --datasets tcga pdgm --out_dir OUTPUT_DIR

BBOX METHOD:
Extract the smallest bounding box that contains the tumor (from the mask).
Expand the box by a margin (e.g., 10–20 voxels in each direction) to include some context.
Crop the image and mask to this box.
Resize the cropped region to a fixed shape (e.g., (80, 96, 80)).
"""
...
from monai.transforms import (
    ...
    ResizeWithPadOrCropd, NormalizeIntensityd, Compose, CropForegroundd
)
...
def patchify(imgs, patch_size):
    ...
    assert D % pz == 0 and H % py == 0 and W % px == 0, "Crop size must be divisible by patch size"
    imgs = imgs.reshape(B, C, D // pz, pz, H // py, py, W // px, px)
    imgs = imgs.permute(0, 2, 4, 6, 1, 3, 5, 7).contiguous()
    N = (D // pz) * (H // py) * (W // px)
    P = pz * py * px
...
def main():
    ...
    parser.add_argument("--crop_size", nargs=3, type=int, default=[80, 96, 80], help="Crop size for tumor bbox (D H W)")
    parser.add_argument("--margin", nargs=3, type=int, default=[10, 10, 10], help="Margin to add to bbox (D H W)")
    ...
    # ---------- Set crop size and margin
    crop_size = tuple(args.crop_size)
    margin = list(args.margin)
