...
class MRISliceDataset(Dataset):
    def __init__(self, nii_paths, label_paths):
        ...
        for img_path, lbl_path in self.pairs:
            ...
            # Use full volume, no cropping
            for i in range(img.shape[2]):
                self.slice_data.append((img[:, :, i], os.path.basename(img_path).replace(".nii.gz", f"_z{i}.npy")))
...
def train(dataset_names, epochs, batch_size, save_ckpt_steps, num_workers, save_dir):
...
if __name__ == "__main__":
    ...
    train(args.datasets, args.epochs, args.batch_size, args.save_ckpt_steps, args.num_workers, args.save_dir)